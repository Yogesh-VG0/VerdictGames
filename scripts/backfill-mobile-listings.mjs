...
if (!IOS_ONLY) {
  ...
  for (const game of games) {
    ...
    try {
      ...
      // Step 1: Try direct package name from existing play_store_url
      ...
      // Step 2: If no direct match, search by title with tiered matching
      if (!appData) {
        ...
        // Use confidence-tiered matching (≥90% auto, ≥80% + dev match)
        if (bestMatch && isHighConfidenceMatch(game.title, game.developer ?? "", bestMatch.title, bestMatch.developer ?? "", bestSim)) {
          ...
          // Verify it's actually a game, not a utility app
      ...
      // Step 3: Upsert verified listing
      await sql`
      ...
      // Update play_store_url on the game if missing
...
if (!ANDROID_ONLY) {
  ...
  for (const game of games) {
    ...
    try {
      // Apple iTunes Search API — official, no scraping needed
      ...
      if (!res.ok) {
        ...
        stats.ios.failed++;
        await sleep(3500); // Apple rate limit: ~20/min
        continue;
      }
      ...
      // Filter to games (genreId 6014 = Games)
      ...
      // Use confidence-tiered matching (same as Android)
    ...
    await sleep(3500); // Apple rate limit: ~20 calls/min
  }
}
